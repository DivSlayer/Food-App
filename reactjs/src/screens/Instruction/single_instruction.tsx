...
import './single_instruction.scss';
...
const SingleInstruction = () => {
    ...
    const fetchInstruction = async () => {
        let found_transaction: InstructionModel[] | null = await InstructionBloc(accessToken ?? "", uuid).then(item => {
            setIsLoading(false);
            return item;
        });
        if (found_transaction && found_transaction.length > 0) {
            setInstruction(found_transaction[0]);
        ...
        if (selectedFile) {
            ...
            try {
                ...
                const response = await axios.put(link, formData, {
                    ...
                    setInstruction(InstructionModel.fromJson(item.data));
                    return item;
                })
                CustomToast.success()
            ...
            <div className='single-instruction animate__animated animate__fadeInDown'>
                {
                    isLoading ? <div className='loading'>
                        ...
                        <div className="details-holder">
                            ...
                            <div className='image' style={{backgroundImage: `url(${instruction?.image})`}}>
